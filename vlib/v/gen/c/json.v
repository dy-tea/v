...
'
		...
		map_keys_call := if utyp.is_ptr() {
			'builtin__map_keys(val)'
		} else {
			'builtin__map_keys(&val)'
		}
		map_get_call := if utyp.is_ptr() {
			'builtin__map_get(val, &key, &(${styp_v}[]) { ${zero} } )'
		} else {
			'builtin__map_get(&val, &key, &(${styp_v}[]) { ${zero} } )'
		}
		return '
		o = cJSON_CreateObject();
		Array_${styp} ${keys_tmp} = ${map_keys_call};
		for (${ast.int_type_name} i = 0; i < ${keys_tmp}.len; ++i) {
			${key}
			cJSON_AddItemToObject(o, (char*) key.str, ${fn_name_v} ( *(${styp_v}*) ${map_get_call} ) );
		}
		builtin__array_free(&${keys_tmp});
'
	}
